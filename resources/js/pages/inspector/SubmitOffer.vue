...
<template>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2">
            <PageCard :title="t('dashboard.inspectorPages.offerFor', { vehicle: request.vehicle })" :subtitle="`${request.service} · ${request.ort} · ${request.number}`">
                <form class="space-y-5 p-5 sm:p-6" @submit.prevent="submit">
                    <div>
                        <label for="price" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('dashboard.inspectorPages.yourPrice') }}</label>
                        <div class="relative">
                            <input id="price" v-model="form.price" type="text" inputmode="decimal" required :placeholder="t('dashboard.inspectorPages.pricePlaceholder')"
                    ...
                    <div>
                        <label for="est" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('dashboard.inspectorPages.estimatedDone') }}</label>
                    ...
                    <div>
                        <label for="msg" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('dashboard.inspectorPages.messageToCustomer') }}</label>
                        <textarea id="msg" v-model="form.message" rows="4" :placeholder="t('dashboard.inspectorPages.messagePlaceholder')"
                    ...
                    <button type="submit" :disabled="form.processing" class="w-full rounded-pill bg-green-500 py-3.5 font-bold text-white transition hover:bg-green-600 disabled:opacity-60">
                        {{ t('dashboard.inspectorPages.submitOfferBinding') }}
                    </button>
                </form>
            </PageCard>
        </div>
