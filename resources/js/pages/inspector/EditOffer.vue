...
const form = useForm({
    ...
    estimated_date: props.offer.estimated_date ?? '',
    message: props.offer.message ?? '',
});
...
function submit() {
    form.transform((d) => ({ ...d, price: String(d.price).replace(',', '.') }))
        .put(`/inspector/requests/${props.request.id}/offer`);
}
</script>

<template>
    <Head><title>{{ 'Angebot bearbeiten' }}</title></Head>
