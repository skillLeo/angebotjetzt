...
<template>
    <Head><title>Auszahlung anfordern</title></Head>
    ...
    <Link href="/gutachter/wallet" class="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-ink-500 hover:text-navy-700">
        <ArrowLeft :size="16" aria-hidden="true" /> Zurück zum Wallet
    </Link>

    <div class="grid gap-6 lg:grid-cols-2">
        <PageCard title="Auszahlung anfordern" :subtitle="`Verfügbar: ${formatEuro(available - pendingPayout)}`">
            <form class="space-y-5 p-5 sm:p-6" @submit.prevent="submit">
                <div>
                    <label for="amount" class="mb-1.5 block text-sm font-semibold text-navy-700">Betrag (EUR) *</label>
                    <input id="amount" v-model="form.amount" type="text" inputmode="decimal" required :placeholder="`max. ${maxPayout.toFixed(2).replace('.', ',')}`" class="w-full rounded-card border border-ink-300 px-4 py-3 text-[15px] focus:border-green-500 focus:outline-none" />
                    <p v-if="form.errors.amount" class="mt-1 text-sm text-red-600">{{ form.errors.amount }}</p>
                </div>
                <div>
                    <label for="holder" class="mb-1.5 block text-sm font-semibold text-navy-700">Kontoinhaber *</label>
                ...
                <div>
                    <label for="iban" class="mb-1.5 block text-sm font-semibold text-navy-700">IBAN *</label>
                ...
                <div>
                    <label for="bic" class="mb-1.5 block text-sm font-semibold text-navy-700">BIC (optional)</label>
                ...
                <p class="text-xs text-ink-500">Auszahlungen werden manuell bearbeitet und in der Regel innerhalb weniger Werktage überwiesen.</p>
            </form>
        </PageCard>

        <PageCard title="Auszahlungsverlauf">
            <div v-if="history.length" class="divide-y divide-ink-100">
                <div v-for="p in history" :key="p.id" class="flex items-center justify-between gap-3 px-5 py-4 sm:px-6">
                    <div>
                        ...
                        <p class="text-xs text-ink-500">Angefordert {{ p.requested }}<span v-if="p.paid"> · Bezahlt {{ p.paid }}</span></p>
                    </div>
                    <StatusBadge :status="p.status" />
                </div>
            </div>
            <p v-else class="px-5 py-8 text-center text-sm text-ink-500">Noch keine Auszahlungen angefordert.</p>
        </PageCard>
    </div>
</template>
