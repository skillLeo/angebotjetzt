...
<template>
    <Head><title>{{ 'Servicegebiet' }}</title></Head>

    <div class="grid gap-6 lg:grid-cols-2">
        <PageCard :title="'Mein Servicegebiet'" :subtitle="'Städte und PLZ-Bereiche, in denen Sie Anfragen erhalten.'">
            <div v-if="areas.length" class="divide-y divide-ink-100">
                <div v-for="a in areas" :key="a.id" class="flex items-center justify-between gap-3 px-5 py-4 sm:px-6">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="font-semibold text-navy-700">{{ a.type === 'city' ? a.city : `PLZ ${a.from} – ${a.to}` }}</p>
                            <p class="text-xs text-ink-500">{{ a.type === 'city' ? 'Stadt' : 'PLZ-Bereich' }}</p>
                        </div>
                    </div>
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded-pill text-ink-500 transition hover:bg-red-50 hover:text-red-600" :aria-label="'Entfernen'" @click="remove(a.id)">
                        <Trash2 :size="17" aria-hidden="true" />
                    </button>
                </div>
            </div>
            <p v-else class="px-5 py-8 text-center text-sm text-ink-500">{{ 'Noch keine Servicegebiete definiert.' }}</p>
        </PageCard>

        <PageCard :title="'Servicegebiet hinzufügen'">
            <form class="space-y-5 p-6 sm:p-8" @submit.prevent="submit">
                <div class="flex gap-2">
                    <button type="button" class="flex-1 rounded-pill py-2.5 text-sm font-bold transition" :class="type === 'city' ? 'bg-navy-700 text-white' : 'bg-sand-50 text-ink-700'" @click="type = 'city'">{{ 'Stadt' }}</button>
                    <button type="button" class="flex-1 rounded-pill py-2.5 text-sm font-bold transition" :class="type === 'postal_range' ? 'bg-navy-700 text-white' : 'bg-sand-50 text-ink-700'" @click="type = 'postal_range'">{{ 'PLZ-Bereich' }}</button>
                </div>

                <div v-if="type === 'city'">
                    <label for="city" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ 'Stadt' }}</label>
                    <input id="city" v-model="form.city_name" type="text" :placeholder="'z. B. Köln'" class="w-full rounded-card border border-ink-300 px-4 py-3 text-[15px] focus:border-green-500 focus:outline-none" />
                    <p v-if="form.errors.city_name" class="mt-1 text-sm text-red-600">{{ form.errors.city_name }}</p>
                </div>
                <div v-else class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="from" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ 'Von PLZ' }}</label>
                    ...
                    <div>
                        <label for="to" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ 'Bis PLZ' }}</label>
