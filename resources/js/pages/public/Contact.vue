...
function submit() {
    form.post('/kontakt', {
        ...
        onSuccess: () => {
            form.reset();
            toast.success(t('public.contact.successToast'));
        },
    });
}
</script>
...
<template>
    <Head>
        <title>{{ t('public.contact.metaTitle') }}</title>
        <meta name="description" :content="t('public.contact.metaDescription')" />
    </Head>

    <section class="bg-sand-100 px-4 py-14 sm:px-6 lg:px-8 lg:py-16">
        <div class="mx-auto max-w-7xl">
            <p class="text-eyebrow mb-4 text-green-600">{{ t('public.contact.eyebrow') }}</p>
            <h1 class="text-hero max-w-2xl text-navy-700">{{ t('public.contact.title') }}</h1>
        </div>
    </section>
        ...
        <div class="mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[1fr_1.2fr] lg:px-8">
            <div class="space-y-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="font-display font-bold text-navy-700">{{ t('public.contact.email') }}</p>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="font-display font-bold text-navy-700">{{ t('public.contact.phone') }}</p>
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <p class="font-display font-bold text-navy-700">{{ t('public.contact.address') }}</p>
            ...
            <form class="rounded-panel border border-ink-100 bg-white p-7 shadow-card" @submit.prevent="submit">
                <div class="grid gap-5 sm:grid-cols-2">
                    <div>
                        <label for="c-name" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('public.contact.name') }}</label>
                    ...
                    <div>
                        <label for="c-email" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('public.contact.email') }}</label>
                ...
                <div class="mt-5">
                    <label for="c-subject" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('public.contact.subject') }}</label>
                ...
                <div class="mt-5">
                    <label for="c-message" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('public.contact.message') }}</label>
