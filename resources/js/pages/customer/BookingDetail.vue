...
function submitReview() {
    ...
    reviewForm.post(`/account/bookings/${props.booking.id}/review`, {
        ...
        onSuccess: () => {
            ...
            toast.success(t('dashboard.customerPages.reviewThanks'));
        },
    });
}
</script>
...
<template>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="space-y-6 lg:col-span-2">
            <PageCard :title="`${booking.vehicle} · ${booking.service}`" :subtitle="booking.number">
                ...
                <div class="p-5 sm:p-6">
                    <p class="text-sm text-ink-500">{{ t('dashboard.customerPages.yourInspector') }}</p>
                    ...
                    <div class="mt-4 flex flex-col gap-3 sm:flex-row">
                        ...
                        <a v-if="booking.inspectorEmail" :href="`mailto:${booking.inspectorEmail}`" class="inline-flex items-center gap-2 rounded-pill border border-ink-300 px-5 py-2.5 text-sm font-bold text-navy-700 transition hover:border-navy-700">
                            <Mail :size="16" aria-hidden="true" /> {{ t('dashboard.customerPages.email') }}
                        </a>
                    </div>
            ...
            <PageCard v-if="['completed_by_inspector', 'confirmed'].includes(booking.status) && !booking.hasReview" :title="t('dashboard.customerPages.reviewThisBooking')">
                <div class="p-5 sm:p-6">
                    <button v-if="!showReview" type="button" class="rounded-pill bg-green-500 px-6 py-2.5 text-sm font-bold text-white transition hover:bg-green-600" @click="showReview = true">
                        {{ t('dashboard.customerPages.rateNow') }}
                    </button>
                    <div v-else>
                        <div class="flex items-center gap-2">
                            <button v-for="i in 5" :key="i" type="button" @click="rating = i" :aria-label="t('dashboard.customerPages.starsLabel', { n: i })">
                                <StarRating :rating="i <= rating ? 5 : 0" :size="26" class="pointer-events-none" />
                            </button>
                        </div>
                        <textarea v-model="reviewForm.comment" rows="4" :placeholder="t('dashboard.customerPages.reviewPlaceholder')" class="mt-4 w-full rounded-card border border-ink-300 px-4 py-3 text-[15px] focus:border-green-500 focus:outline-none" />
                        <div class="mt-4 flex gap-3">
                            <button type="button" :disabled="reviewForm.processing" class="rounded-pill bg-green-500 px-6 py-2.5 text-sm font-bold text-white disabled:opacity-60" @click="submitReview">{{ t('dashboard.customerPages.sendReview') }}</button>
                            <button type="button" class="rounded-pill px-6 py-2.5 text-sm font-bold text-ink-500" @click="showReview = false">{{ t('dashboard.customerPages.cancel') }}</button>
                        </div>
                    </div>
                </div>
            </PageCard>
        ...
        <div>
            <PageCard :title="t('dashboard.customerPages.paymentOverview')">
                <div class="space-y-3 p-5 text-sm sm:p-6">
                    <div class="flex justify-between"><span class="text-ink-500">{{ t('dashboard.customerPages.bookingValue') }}</span><span class="font-bold text-navy-700">{{ formatEuro(booking.price) }}</span></div>
                    <div class="flex justify-between border-t border-ink-100 pt-3">
                        <span class="text-ink-500">{{ t('dashboard.customerPages.payment') }}</span>
