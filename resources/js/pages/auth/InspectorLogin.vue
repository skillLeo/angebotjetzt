...
<template>
    <Head><title>{{ t('auth.inspectorLogin.headTitle') }}</title></Head>

    <SplitAuthShell
        :quote="t('auth.inspectorLogin.quote')"
        quote-author="Thomas Bergmann, Kfz-Sachverständiger"
    >
        <h1 class="font-display text-3xl font-extrabold text-navy-700">{{ t('auth.inspectorLogin.heading') }}</h1>
        <p class="mt-2 text-ink-500">{{ t('auth.inspectorLogin.subtitle') }}</p>

        <form class="mt-8 space-y-5" @submit.prevent="submit">
            <div>
                <label for="email" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('auth.inspectorLogin.email') }}</label>
            ...
            <div>
                <label for="password" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('auth.inspectorLogin.password') }}</label>
            ...
            <label class="flex items-center gap-2 text-sm text-ink-700">
                <input v-model="form.remember" type="checkbox" class="h-4 w-4 accent-green-500" /> {{ t('auth.inspectorLogin.rememberMe') }}
            </label>
