...
<template>
    <Head><title>{{ t('auth.adminLogin.headTitle') }}</title></Head>

    <SplitAuthShell :quote="t('auth.adminLogin.quote')">
        <h1 class="font-display text-3xl font-extrabold text-navy-700">{{ t('auth.adminLogin.heading') }}</h1>
        <p class="mt-2 text-ink-500">{{ t('auth.adminLogin.subtitle') }}</p>

        <form class="mt-8 space-y-5" @submit.prevent="submit">
            <div>
                <label for="email" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('auth.adminLogin.email') }}</label>
            ...
            <div>
                <label for="password" class="mb-1.5 block text-sm font-semibold text-navy-700">{{ t('auth.adminLogin.password') }}</label>
            ...
            <label class="flex items-center gap-2 text-sm text-ink-700">
                <input v-model="form.remember" type="checkbox" class="h-4 w-4 accent-green-500" /> {{ t('auth.adminLogin.rememberMe') }}
            </label>
