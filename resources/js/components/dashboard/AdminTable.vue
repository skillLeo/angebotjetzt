...
<template>
    <div>
        ...
        <table class="hidden w-full md:table">
            ...
            <tbody>
                <tr v-for="(row, i) in rows" :key="rowKey ? String(row[rowKey]) : i" class="border-b border-ink-100 transition hover:bg-sand-50">
                    <td
                        v-for="col in columns"
                        :key="col.key"
                        class="px-4 py-3 text-sm text-navy-700"
                        :class="{ 'text-right': col.align === 'right', 'text-center': col.align === 'center' }"
                    >
        ...
        <div class="divide-y divide-ink-100 md:hidden">
            <div v-for="(row, i) in rows" :key="rowKey ? String(row[rowKey]) : i" class="space-y-2 px-4 py-4">
        ...
        <p v-if="!rows.length" class="px-4 py-10 text-center text-sm text-ink-500">Keine Einträge vorhanden.</p>
    </div>
</template>
